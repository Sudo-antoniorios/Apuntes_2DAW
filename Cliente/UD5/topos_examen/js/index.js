...
if (!localStorage.getItem("jugador")) {
  ...
  mostrarJuego(JSON.parse(localStorage.getItem("jugador")));
}
...
function crearFormulario() {
  ...
  columnaTabla1.append(
    radioTablaBlue,
    document.createElement("br"),
    divColorBlue
  );
  columnaTabla2.append(
    radioTablaGreen,
    document.createElement("br"),
    divColorGreen
  );
  columnaTabla3.append(
    radioTablaYellow,
    document.createElement("br"),
    divColorYellow
  );
  columnaTabla4.append(
    radioTablaPink,
    document.createElement("br"),
    divColorPink
  );
...
function mostrarJuego(jugador) {
  document.body.style.backgroundImage = "imagenes/fondo_topos.png";
  ...
  btnCerrar.addEventListener("click", () => {
    ...
    localStorage.clear();
    document.body.style.backgroundImage = "url('imagenes/fondo_topos.png')";
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      celda.addEventListener("click", () => {
        if (imagen.src.includes("topo.png")) {
          puntuacion++;
...
function iniciarJuego() {
  const celdas = document.querySelectorAll("td img");
  let intervaloTopo = setInterval(() => {
    const indice = Math.floor(Math.random() * celdas.length);
    if (celdas[indice].src.includes("agujero.png")) {
